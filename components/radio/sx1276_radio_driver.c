...
typedef struct
{
    ...
    uint8_t *dataBuffer;
    TimerHandle_t timeOnAirTimerId;
    TimerHandle_t fskRxWindowTimerId;
    TimerHandle_t watchdogTimerId;
...
static void RADIO_RxFSKTimeout(TimerHandle_t xExpiredTimer);
static void RADIO_WatchdogTimeout(TimerHandle_t xExpiredTimer);
...
void RADIO_Init(uint8_t *radioBuffer, uint32_t frequency)
{
    ...
    if (0 == RadioConfiguration.initialized)
    {
        ...
        RadioConfiguration.timeOnAirTimerId = xTimerCreate("timeOnAirTimer", 0, pdFALSE, NULL, NULL);
        RadioConfiguration.fskRxWindowTimerId = xTimerCreate("fskRxWindowTimer", 0, pdFALSE, NULL, RADIO_RxFSKTimeout);
        RadioConfiguration.watchdogTimerId = xTimerCreate("watchdogTimer", 0, pdFALSE, NULL, RADIO_WatchdogTimeout);
